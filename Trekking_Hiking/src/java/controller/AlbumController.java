...
public class AlbumController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            //get share panel
            ArrayList<Share> sharePanel = hdao.getSharePanel();
            request.setAttribute("sharePanel", sharePanel);
            //get pages
            ArrayList<Page> pages = hdao.getPages();
            request.setAttribute("pageList", pages);
            //count image number
            int count = adao.countImage();
            if (count <= 0) {
                request.setAttribute("error", "Diary is not available");
            }
            //get pageSize of albumPage
            ...
            ArrayList<Image> listPhoto = adao.getPhotoWithPaging(index);
            request.setAttribute("listPhoto", listPhoto);
            //set active page
            request.setAttribute("active", "PhotoAlbum");
            request.getRequestDispatcher("PhotoAlbum.jsp").forward(request, response);
        } catch (Exception ex) {
